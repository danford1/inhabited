const data = {
  ...
  cents: null,
  dailyOrbits: 15.54,
  time: new Date(2000, 11, 2, 10, 51, 0, 0, 0) // 11/2/2000 @ 10:51 UTC
};
...
function update() {
  ...
  data.orbits = data.days * data.dailyOrbits;
  // data.roundedCost = Math.round( data.cost / Math.pow(10,9) )

  data.whole = Math.floor(data.orbits)
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  data.fraction = (data.orbits - Math.floor(data.orbits))
    .toFixed(4)
    .split(".")[1];
  updatePies();
  document.getElementById("whole").innerText = data.whole;
  document.getElementById("fraction").innerText = data.fraction;
